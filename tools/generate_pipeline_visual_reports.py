...
def build_sections(config: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    sections.append(("Step 5 Weather", section("Step 5 Weather", "Punktwetter und HOSTRADA-100m-Rasterprodukte.", {"weather rows": int(len(weather)), "weather issues": weather_cards.get("has issues", 0), "raster bands": int(len(raster_qc))}, "Weather status", weather_cards, preview(weather, ["dawn_chorus_id", "weather_exists", "weather_has_issues", "issues"]), weather_notes + (["Raster-QC: " + ", ".join(f"{key}={value}" for key, value in raster_chart.items())] if raster_chart else []))))
