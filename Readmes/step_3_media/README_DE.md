...
Die verbindlichen Abhaengigkeiten, der Scope und die Invalidierungsregeln stehen in `pipeline_steps.json` unter `step_3_0_audio_inventory`, `step_3_0_photo_inventory`, `step_3_1_audio_download`, `step_3_1_photo_download`. Der zentrale Run-Planer gibt nur betroffene IDs weiter und plant globale Schritte nur bei geaenderten Inputs, ergebnisrelevanter Konfiguration oder fehlenden Outputs.

## Konfiguration
Ergebnisrelevante Einstellungen stehen zentral in `config.horeka.json`: `audio_inventory`, `photo_inventory`, `audio_download`, `photo_download`. Pfade, Workerzahlen und fachliche Schwellen werden nicht im Slurm-Script dupliziert.
